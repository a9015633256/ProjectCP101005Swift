//
//  MainClassListTableViewController.swift
...
class MainClassListTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "Cell11", for: indexPath) as? MainClassTableViewCell
        cell?.className.text = classJoin[indexPath.row].classes
        if let id = classJoin[indexPath.row].id{
            cell?.id.text = ("\(id)")
        }
        cell?.teacherName.text = classJoin[indexPath.row].teacher
